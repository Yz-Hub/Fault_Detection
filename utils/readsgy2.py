...
filename = r'E:\conda\jupyter_notebook\ResACEUnet-master\F3\Seismic_data.sgy'
...
# print('--> 数据读取耗时 {:.1f} 秒'.format(time.time()-t0))
...
t0=time.time()
print('sgy 使用 read 读取:')
stream = read(filename, format='SEGY',unpack_trace_headers=True)
print('--> 数据读取耗时 {:.1f} 分钟'.format((time.time()-t0)/60))
...
t0=time.time()
counter = Counter(il)
print('计数耗时 {:.1f} 秒'.format(time.time()-t0))
...
gc.collect()
seis_np = np.zeros((len(inlines),len(xlines),stream[0].stats.npts), dtype=np.float32)  # 声明一个空数组
print(seis_np.shape)
...
t0=time.time()
batch_size = 32
for i in range(0, len(stream), batch_size): # 根据 trace 的 inline 编号和 crossline 编号填充空的 3D 数组。
    ...
    for trace in batch:
        ...
        seis_np[il_idx][xl_idx] = trace.data # 这里 1001 是 thebe 数据集的初始 inline 编号，851 是初始 crossline 编号
print('--> 数据读取耗时 {:.1f} 分钟'.format((time.time()-t0)/60))
...
print('--> 数据保存耗时 {:.1f} 分钟'.format((time.time()-t0)/60))