...
def main(config):
    ...
    logger = create_logger(output_dir=config.train.output, name=config.model.name)
    ...
    for epoch in range(config.train.start_epoch, config.train.epochs):
        ...
        if iou >= max_iou:
            ...
            if patience >= max_patience:
                logger.info(f"Model hasn't improved for {max_patience} epochs. Stopping training")
                break
    ...
    logger.info(f'Training time {time}')
...
if __name__ == '__main__':
    ...
    config.data.path = os.path.abspath(config.data.path)
    print(f"Data_path: {config.data.path}") 

    
    if not os.path.exists(config.data.path):
        raise FileNotFoundError(f"Path_Error: {config.data.path}")
